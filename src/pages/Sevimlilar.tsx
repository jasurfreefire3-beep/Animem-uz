          ...
          {filteredFavorites.map((anime, idx) => (
            ...
            >
              <div className="flex items-center space-x-4 min-w-0 flex-1">
                {/* Poster Thumbnail */}
                <Link to={`/anime/${toSlug(anime.title)}`} title={anime.title} className="w-14 h-18 shrink-0 block group/fav">
                  <div className="w-full h-full p-[1.5px] rounded-xl bg-gradient-to-b from-white/20 via-white/5 to-[#ff006a]/30 group-hover/fav:from-[#ff006a] group-hover/fav:to-purple-600 transition-all duration-300 shadow-sm">
                    <div className="w-full h-full rounded-[10px] overflow-hidden bg-[#0c0c0e] relative">
                      <img loading="lazy" decoding="async" 
                        src={anime.image_url} 
                        alt={anime.title} 
                        title={anime.title} 
                        className="w-full h-full object-cover group-hover/fav:scale-108 transition-transform" 
                      />
                    </div>
                  </div>
                </Link>
